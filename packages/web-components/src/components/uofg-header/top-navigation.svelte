    ...
    {#each links as item (item.text || item.title)}
      <li>
        {#if item.links}
          <Menu
            ...
            buttonClass={twJoin(
              ...
              item.icon && 'aria-expanded:[&_svg]:rotate-180)} aspect-square',
            )}
        ...
        {:else}
          <a
            class={twJoin(
              'hover:bg-grey-muted hover:text-grey-muted-contrast flex h-full items-center justify-center gap-2 px-2 transition-colors',
              item.highlight && 'bg-yellow text-yellow-contrast',
            )}
            href={item.href}
          >
